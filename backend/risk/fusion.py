"""
Fuses the deterministic rule engine (risk/rules.py) with the scoring model
(risk/model.py) into one final risk classification. This function is the
single source of truth for risk level -- the agent/RAG layer downstream only
explains this result, never overrides it.
"""
...
def assess(features: dict) -> dict:
    ...
    # Fusion: weighted blend, but any 2+ triggered rules puts a floor under the fused score
    # so an evaluator can't be lulled by a low model score when multiple hard thresholds are already crossed.
