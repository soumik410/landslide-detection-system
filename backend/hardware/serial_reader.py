"""
Reads raw lines from the Arduino/sensor board over a serial connection.

Expected wire format (one JSON object per line), e.g.:
    {"moisture": 512, "rain": 300, "dist_cm": 45.2, "ax":0.01,"ay":0.02,"az":9.79,"gx":0.1,"gy":0.0,"gz":0.0}

Falls back to a synthetic data generator when SIMULATE_HARDWARE is True or
when the serial port cannot be opened, so the rest of the pipeline can be
developed and demoed without the physical board attached.
"""
import json
import logging
import random
